"""Bank transactions tab (Receipt / Payment / Contra)."""
...
from tally_importer.validator import validate_bank_transactions
from tally_importer.xml_generator import build_bank_xml


class BankTab(ttk.Frame):
    """Tab for importing bank transactions."""

    VOUCHER_TYPES = ("Receipt", "Payment", "Contra")
    ...
    def _build_ui(self) -> None:
        ...
        self._default_vtype = tk.StringVar(value="Payment")
    ...
    def _open_mapping(self) -> None:
        if self._df is None:
            ...
            return
        from tally_importer.mapping import BANK_FIELDS
    ...
    def _validate(self) -> None:
        ...
        valid, errors = validate_bank_transactions(
        ...
        if valid:
            msgs.append(f"\n✔  {len(valid)} valid transactions ready to export.")
    ...
    def _export(self) -> None:
        ...
        if not valid:
            messagebox.showwarning(
                "Export",
                "No valid transactions to export.\nCheck validation messages.",
            )
            return
        if errors:
        ...
        path = filedialog.asksaveasfilename(
            ...
            title="Save Bank Vouchers XML",
        )
        if not path:
            return
        try:
            xml_content = build_bank_xml(valid)
            with open(path, "w", encoding="utf-8") as fh:
                fh.write('<?xml version="1.0" encoding="UTF-8"?>\n')
                fh.write(xml_content)
            messagebox.showinfo(
                "Export Complete",
                f"Exported {len(valid)} bank voucher(s) to:\n{path}",
            )
        except Exception as exc:
