"""Debit Note / Credit Note vouchers tab."""
...
from tally_importer.validator import validate_debit_credit_note
from tally_importer.xml_generator import build_debit_credit_note_xml


class DebitCreditTab(ttk.Frame):
    """Tab for importing Debit Note / Credit Note vouchers."""

    VOUCHER_TYPES = ("Debit Note", "Credit Note")
    ...
    def _build_ui(self) -> None:
        ...
        # Default voucher type – Debit Note or Credit Note
        ttk.Label(top, text="Default Voucher Type:").grid(
            row=2, column=0, sticky="w", pady=3
        )
        self._default_vtype = tk.StringVar(value="Debit Note")
        vtype_combo = ttk.Combobox(
            top,
            textvariable=self._default_vtype,
            values=list(self.VOUCHER_TYPES),
            state="readonly",
            width=14,
        )
    ...
    def _open_mapping(self) -> None:
        if self._df is None:
            ...
            return
        from tally_importer.mapping import DEBIT_CREDIT_NOTE_FIELDS
    ...
    def _validate(self) -> None:
        ...
        valid, errors = validate_debit_credit_note(
        ...
        if valid:
            vtype = self._default_vtype.get()
            msgs.append(f"\n✔  {len(valid)} valid {vtype} entries ready to export.")
    ...
    def _export(self) -> None:
        ...
        path = filedialog.asksaveasfilename(
            ...
            title=f"Save {vtype} Vouchers XML",
        )
        if not path:
            return
        try:
            xml_content = build_debit_credit_note_xml(valid)
            with open(path, "w", encoding="utf-8") as fh:
                fh.write('<?xml version="1.0" encoding="UTF-8"?>\n')
                fh.write(xml_content)
            messagebox.showinfo(
                "Export Complete",
                f"Exported {len(valid)} {vtype} voucher(s) to:\n{path}",
            )
        except Exception as exc:
