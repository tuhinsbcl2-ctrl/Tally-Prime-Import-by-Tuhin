...
from tally_importer.validator import validate_journal_entries
from tally_importer.xml_generator import build_journal_xml


class JournalTab(ttk.Frame):
    """Tab for importing journal entries."""
    ...
    def _open_mapping(self) -> None:
        if self._df is None:
            ...
            return
        from tally_importer.mapping import JOURNAL_ENTRY_FIELDS
    ...
    def _validate(self) -> None:
        ...
        valid, errors = validate_journal_entries(rows, self._mapping)
        msgs: list[str] = []
        if errors:
            for e in errors:
                msgs.append(f"Row {e['row']}: " + "; ".join(e["errors"]))
        if valid:
            msgs.append(f"\n✔  {len(valid)} valid journal entries ready to export.")
    ...
    def _export(self) -> None:
        ...
        if errors:
            ...
            proceed = messagebox.askyesno(
                ...
                f"{detail}\n\n"
                f"Export the {len(valid)} valid entries?",
            )
            if not proceed:
                return
        ...
        path = filedialog.asksaveasfilename(
            ...
            title="Save Journal Entries XML",
        )
        if not path:
            return
        try:
            xml_content = build_journal_xml(valid)
            with open(path, "w", encoding="utf-8") as fh:
                fh.write('<?xml version="1.0" encoding="UTF-8"?>\n')
                fh.write(xml_content)
            messagebox.showinfo(
                "Export Complete", f"Exported {len(valid)} journal entry(s) to:\n{path}"
            )
        except Exception as exc:
