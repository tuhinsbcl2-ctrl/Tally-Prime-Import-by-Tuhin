"""Purchase itemwise vouchers tab."""
...
from tally_importer.validator import validate_purchase_itemwise
from tally_importer.xml_generator import build_purchase_itemwise_xml


class PurchaseItemwiseTab(ttk.Frame):
    """Tab for importing itemwise purchase invoices."""
    ...
    def _open_mapping(self) -> None:
        if self._df is None:
            ...
            return
        from tally_importer.mapping import PURCHASE_ITEMWISE_FIELDS

        dlg = MappingDialog(
            self,
            "purchase_itemwise",
            PURCHASE_ITEMWISE_FIELDS,
    ...
    def _validate(self) -> None:
        ...
        valid, errors = validate_purchase_itemwise(
        ...
        if valid:
            msgs.append(f"\n✔  {len(valid)} valid purchase invoices (itemwise) ready to export.")
    ...
    def _export(self) -> None:
        ...
        if errors:
            ...
            proceed = messagebox.askyesno(
                ...
                f"{detail}\n\n"
                f"Export the {len(valid)} valid invoices?",
            )
            if not proceed:
                return
        ...
        path = filedialog.asksaveasfilename(
            ...
            title="Save Purchase Itemwise Vouchers XML",
        )
        if not path:
            return
        try:
            xml_content = build_purchase_itemwise_xml(valid)
            with open(path, "w", encoding="utf-8") as fh:
                fh.write('<?xml version="1.0" encoding="UTF-8"?>\n')
                fh.write(xml_content)
            messagebox.showinfo(
                "Export Complete", f"Exported {len(valid)} purchase invoice(s) to:\n{path}"
            )
        except Exception as exc:
