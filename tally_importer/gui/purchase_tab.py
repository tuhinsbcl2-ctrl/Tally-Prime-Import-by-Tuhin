...
from tally_importer.validator import validate_purchase
from tally_importer.xml_generator import build_purchase_xml


class PurchaseTab(ttk.Frame):
    """Tab for importing purchase invoices."""
    ...
    def _open_mapping(self) -> None:
        if self._df is None:
            ...
            return
        from tally_importer.mapping import PURCHASE_FIELDS
    ...
    def _validate(self) -> None:
        ...
        if valid:
            msgs.append(f"\n✔  {len(valid)} valid purchase entries ready to export.")
    ...
    def _export(self) -> None:
        ...
        path = filedialog.asksaveasfilename(
            ...
            title="Save Purchase Vouchers XML",
        )
        if not path:
            return
        try:
            xml_content = build_purchase_xml(valid)
            with open(path, "w", encoding="utf-8") as fh:
                fh.write('<?xml version="1.0" encoding="UTF-8"?>\n')
                fh.write(xml_content)
            messagebox.showinfo(
                "Export Complete",
                f"Exported {len(valid)} purchase voucher(s) to:\n{path}",
            )
        except Exception as exc:
